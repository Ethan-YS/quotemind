...
> 中文工作稿 v0.1 —— 定稿后出英文版（英文版为最终交付物）
> 配套原型：QuoteMind —— AI RFQ→Quotation 流水线（见仓库 README）
...
1. **报价既慢又贵，还大多打了水漂。** 定制件报价周期通常 5~10 个工作日；而行业典型中标率 20~30%，意味着 **70~80% 的报价工程投入不产生任何收入**。报价慢本身还压低中标率——客户往往把订单给响应最快的合格供应商。这是"成本"与"营收"两头同时失血的环节。
...
flowchart LR
    ...
    RC -->|通过, 带置信度| GATE{{人工审核门<br/>高: 抽检 · 中: 确认 · 低: 强制介入}}
    RC -->|矛盾/缺字段| GATE
...
- **模型无关抽象层**：Gemini / Claude / GPT / 开源本地模型可切换。这不是技术洁癖——德国客户对**数据主权（GDPR、图纸即商业机密）**极度敏感，"可切换到 EU region 或 on-prem 开源模型部署"是签单的前提条件，必须是架构的一等公民。
- 图纸理解不迷信端到端 LLM：**vision LLM 抽取 + 规则校验（尺寸闭环检查、标题栏字段完整性）** 双保险。
...
- **HITL 三分法**（每个 AI 输出按置信度分档）：
  - 高置信 → 默认通过，抽检
  - 中置信 → 必须人工确认后进入下一阶段
  - 低置信 / 检出矛盾 → 强制人工介入，AI 只列证据
- 人的每次修正都被记录，回流为评测集与微调数据——**系统随使用变准**。
...
**3 年（基准档）：收益 ~€9.0M vs 成本 ~€1.9M → 净收益 ~€7.1M。**
口径写清楚以免歧义：**净 ROI =（收益−成本）/ 成本 ≈ 374%**；**收益成本比 ≈ 4.7×**；**回本周期 < 12 个月**。
保守档（40% 提效、知识流减半）：3 年收益 ~€6.2M，净 ROI ≈ 226%（收益成本比 ≈ 3.3×）——**保守档也成立**。

> 口径说明（防重复计算）：RFQ 链的 69,000h 已包含报价场景下的审图工时；表中"图纸审查 30,000h"指**不经过报价流程**的新品设计图与工程变更图，两者互斥不重叠。

### 4.3 报价周期与营收上行（不计入 ROI，作为 upside）

- 报价周期：**5~10 天 → 1~2 天**（简单件当天）。
- 响应速度是中标率的强变量：以 €450M 营收、报价转化口径估算，**中标率每 +1pp ≈ 千万欧级年营收增量**。哪怕只兑现零头，也超过整个平台成本。故意不计入 ROI 主表——让 ROI 用最硬的成本口径也能自洽。

### 4.4 人员口径（对德企要讲对）

**不按裁员讲，按产能重配讲**：基准档年化 €3.0M 折合约 **35 FTE 等效产能**（其中 RFQ 链约 24 FTE、知识检索约 5 FTE、图纸与 ECR 约 7 FTE），投向新品开发、客户定制项目与工艺改进——这些是现在"想做没人做"的事。工程师老龄化 + 技工缺口背景下，"用 AI 放大现有团队"比"用 AI 替换团队"既更真实，也更容易拿到工程师群体的配合（采纳率本身就是 ROI 的前提）。
...
- 出口指标（Gate）：特征抽取准确率 ≥95%（人审前）、相似件 Top-5 命中率 ≥80%、试点组报价周期 ≥50% 缩短、工程师满意度
...
| 数据主权（GDPR/商业机密） | EU region / on-prem 部署选项；图纸不出客户边界 |
...
原型实现主线的端到端：**样例 RFQ + 工程图纸 → 特征卡抽取（vision LLM）→ 相似件检索（结构化特征匹配，演示混合检索中的结构化通道）→ BOM 草稿 → 工艺路线草稿 → 成本分解与报价书草稿**，每一阶段均为人工可编辑的 HITL 检查点。架构决策、demo mode 设计与运行方式见仓库 README。
