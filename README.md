...
6. **每条 AI 产出都带依据（basis）与置信度。** BOM 行引用了哪个历史零件、工时按哪条产线校准、哪些字段必须人工确认——可追溯性优先于流畅性。工程师的每次修正都被记录并计数（审批栏实时显示"已修正 N 处"），修正值传入下一阶段——这就是生产版"人工修正回流为评测集"的雏形。
7. **模型无关抽象层。** 流水线不知道自己在 live 还是 demo mode；换 Gemini/Claude/本地开源模型只动 `llm.py` 一个文件。对德国客户，"可切换到 EU region / on-prem 部署"是签单前提，抽象层是架构的一等公民。
8. **Demo 缓存 = 真实模型输出的快照**：由 codex-cli 后端（gpt-5.5）通过 `scripts/regen_demo_cache.py` 真实跑一遍流水线生成，四阶段输出过 schema 校验后落盘。每条成本都锚定零件库参考件实绩（basis 字段可逐行核对），不是演示用的假数字。
