...
flowchart TB
    ...
    subgraph 编排层["Agent 编排层（状态机 + 检查点）"]
        O[RFQ Orchestrator] --> G1[Drawing Agent]
        O --> G2[Knowledge Agent]
        O --> G3[BOM Agent]
        O --> G4[Process Agent]
        O --> G5[Quotation Agent]
        H{{人工审核门<br/>每阶段 HITL Gate}}
    end
    subgraph 知识层
...
**3 年 ROI（基准）：收益 ~€9.0M vs 成本 ~€1.9M → 净收益 ~€7.1M，ROI ≈ 3.7x，回本周期 < 12 个月。**
保守档（40% 提效、知识流减半）：3 年收益 ~€6.2M，ROI ≈ 2.3x——**保守档也成立**。
...
**不按裁员讲，按产能重配讲**：释放的 ~25 FTE 等效产能投向新品开发、客户定制项目与工艺改进——这些是现在"想做没人做"的事。工程师老龄化 + 技工缺口背景下，"用 AI 放大现有团队"比"用 AI 替换团队"既更真实，也更容易拿到工程师群体的配合（采纳率本身就是 ROI 的前提）。
