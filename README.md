...
4. **每条 AI 产出都带依据（basis）与置信度。** BOM 行引用了哪个历史零件、工时按哪条产线校准、哪些字段必须人工确认——可追溯性优先于流畅性。工程师的每次修正都被记录并计数（审批栏实时显示"已修正 N 处"），修正值传入下一阶段——这就是生产版"人工修正回流为评测集"的雏形。
...
6. **Demo 缓存 = 真实模型输出的快照**：由 codex-cli 后端（gpt-5.5）通过 `scripts/regen_demo_cache.py` 真实跑一遍流水线生成，四阶段输出过 schema 校验后落盘。每条成本都锚定零件库参考件实绩（basis 字段可逐行核对），不是演示用的假数字。
